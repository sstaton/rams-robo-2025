...
def autonomousredleft():
    ...
    while(time_now < start_time + 1.0):
        ...
        wait(10, MSEC)
        time_now = time.time()
    drive_straight(5, 8, 8)
    wait(200,MSEC)
    drive_turn(-3.7, 3.5, 3, 10, False)
    aligner.set(True)
    unloader.set(False)
    drive_straight(42, 68, 50)
    outtake2.spin(FORWARD)
    descorer.set(True)
...
def autonomousblueleft():
    ...
    wait(2100, MSEC) # let the balls out for a couple secs
    drive_r.spin_for(FORWARD, 920, DEGREES) # only spin the right side so we make a wide arc
...
if do_testing:
    ...
    field_controller = Competition(opcontrol, autonomousblueright)
